...
//Regina - Here is my final, I will also upload my other examples that I built upon. I was pulling and changing code from the book  "Mastering Frameworks"

//since my code didn't work when I attempted to add the rotating sphere, I'm going to try and rebuild this and change it enough//
...
void Params::setup() {
    eCenter = ofPoint (ofGetWidth()/2+200, ofGetHeight ()/2);
    eRad =800; // how large the radius will be
    velRad = 50; //the speed of the radius
    lifeTime = 3.0; //how long the Params will be alive
    
    rotate = 0; // the degree of rotation
...
ofPoint randomPointInCircle (float maxRad) {
    ...
    float rad = ofRandom (20, maxRad); //the float radius is going to be openFrameworks Random, at a location of x=0 and the maximum Radius for y.
    
    float angle = ofRandom (100,M_TWO_PI); //not sure if I get this entirely, but I think the angle is also going to be Random, with an X position of zero, and the Y position to incorporate two PI (3.14)
...
void Particle::draw () {
    if (live) {
        ...
        float hue = ofMap (time, 0, lifeTime, 30,0);
        color.setHue (hue);
        ofSetColor (color);
...
void ofApp::setup(){
   ...
   // image.load("image8.jpg");
    ...
    history = 0.3; //i believe this is referencing the trail history since the value of 1 would be infiinte and the informaiton I read earlier said that 0.8 would be a long trail
...
//
//    //reread... just confired that dt is a time step that computes the value of the current time and the previous time that the particle system was called to update.
...
//    ofPushMatrix();
//    ofTranslate(0,0);//image.ofGetWidth()/2, image.ofGetHeight()/2, 0);//move pivot to centre
//    ofRotate(ofGetFrameNum() * .01, 0, 0, 1);//rotate from centre
//    ofPushMatrix();
//    image.draw(0,0);//-image.ofGetWidth()/2,-image.ofGetHeight()/2);//move back by the centre offset
//    ofPopMatrix();
//    ofPopMatrix();
    ...
    //image rotation -----//
//    ofPushMatrix();
//    ofTranslate(100,100);//image.ofGetWidth()/2, image.ofGetHeight()/2, 0);//move pivot to centre
//    ofRotate(ofGetFrameNum() * .01, 0, 0, 1);//rotate from centre
//    ofPushMatrix();
//    image.draw(20, 20);//-image.ofGetWidth()/2,-image.ofGetHeight()/2);//move back by the centre offset
//    ofPopMatrix();
//    ofPopMatrix();
...
//----All this below was from part 1 I had problems with part 2 so going to rewrite the code above ----------------
